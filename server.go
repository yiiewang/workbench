...
func NewServer(db *DB, serveDir string, routeConfig map[string]string, tokenSecret []byte, logFile string) *Server {
	return &Server{
		db:          db,
		serveDir:    serveDir,
...
// handleRouteRedirect 检查路由映射，处理重定向和特殊指令
func (s *Server) handleRouteRedirect(w http.ResponseWriter, r *http.Request) bool {
	target, exists := s.routeConfig[r.URL.Path]
	if !exists {
		return false
	}

	if target == "__listdir__" {
		s.listDirectory(w, r, s.serveDir)
		return true
	}
...
func (s *Server) handleOrgMembers(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, OrgMembersResult{Members: members})
}
...
func (s *Server) handleMe(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]interface{}{
		"userId": userID,
		"orgId":  orgID,
		"exp":    time.Now().Unix() + tokenExpiryDays*86400,
	})
}
...
func (s *Server) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	token := generateToken(req.UserID, s.tokenSecret)
	writeJSON(w, http.StatusOK, map[string]interface{}{
		"token": token,
...
func (s *Server) handleStatic(w http.ResponseWriter, r *http.Request) {
	...
	for filePath, displayName := range s.routeConfig {
	...
	// 构造文件系统路径
	fsPath := filepath.Join(s.serveDir, filepath.Clean(r.URL.Path))
...
func writeJSON(w http.ResponseWriter, status int, data interface{}) {
	...
	if err := json.NewEncoder(w).Encode(data); err != nil {
		log.Printf("write json failed, err=%v", err)
	}
}

// readJSON 从请求体读取 JSON
